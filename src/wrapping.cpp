...
int wrap(char output[3][32], char input[], int start)
{
    ...
    for (int i = start; i < strlen(input) && currStr < 4; i++)
    {
        //if currStr is 4, return i
        if (currStr == 4)
        {
            return i;
        }
        ...
        if (pointer == 31)
        {
            ...
            else if (output[currStr][pointer] == ' ' || output[currStr][pointer] == '/0'){}
            //if the next character is a space or the end of the input string, increment i an extra time to skip it
            else if (input[i + 1] == ' ' || input[i + 1] == '/0')
            {
                i++;
            }
            //if the prior conditions were false, this is the middle of a word, move i back to the last space
            else
            {
                while(input[i] != ' ')
                {
